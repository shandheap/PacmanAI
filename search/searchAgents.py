...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        if problem.goal[corners.index(corner)] != True:
            positionToCorner = euclideanFunction(problem.startingPosition, corner)
            distancesToCorners.append(positionToCorner)
            forest.makeSet(corner)
            for otherCorner in corners:
                if corner != otherCorner and problem.goal[corners.index(otherCorner)] != True:
                    cornerSpacing = mazeDistance(corner, otherCorner, problem.startingGameState)
                    edgeQueue.push((corner, otherCorner, cornerSpacing), cornerSpacing)
    while not edgeQueue.isEmpty():
        edge = edgeQueue.pop()
        if forest.findLabel(edge[0]) != forest.findLabel(edge[1]):
            mstSum += edge[2]
            forest.join(edge[0], edge[1])
    if distancesToCorners != []:
        closestCorner = min(distancesToCorners)
    else:
        closestCorner = 0
    return mstSum + closestCorner
...
class FoodSearchProblem:
    ...
    def __init__(self, startingGameState):
        ...
        for foodLocation in foodPositions:
            self.heuristicInfo[foodLocation] = []
            for otherFood in foodPositions:
                if foodLocation != otherFood:
                    distance = mazeDistance(foodLocation, otherFood, self.startingGameState)
                    self.heuristicInfo[foodLocation].append((otherFood, distance))
    def getStartState(self):
        return self.start
...
def foodHeuristic(state, problem):
    ...
    for foodLocation in foodPositions:
        ...
        for otherFood in foodPositions:
            if foodLocation != otherFood:
                for item in problem.heuristicInfo[foodLocation]:
                    if otherFood == item[0]:
                        distance = item[1]
                        break
